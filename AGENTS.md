...
- Build tool: Stack (resolver `lts-24.28`). Project metadata in `package.yaml` and `hkforum.cabal`.
...
- Default DB path: `data/hkforum.sqlite3`.
- Storage backend:
...
- Build: `stack build`
- Run (prod-ish): `stack run hkforum`
- Run (dev flags): `stack build --flag hkforum:dev && stack exec hkforum`
- Tests: `stack test`
