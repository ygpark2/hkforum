...
postAdminCompaniesR = do
    ...
    if T.null name
        ...
        else if T.null description
            then setMessage "Company description is required."
            else do
...
postAdminCompanyCategoriesR = do
    ...
    if T.null name
        then setMessage "Category name is required."
            ...
            inserted <- runDB $ insertBy $ CompanyGroup name mDescription adminId now
            case inserted of
                Left _ -> setMessage "Company category already exists."
        ...
        "update" -> do
            ...
            if T.null name
                then setMessage "Category name is required."
                    ...
                    mExisting <- runDB $ getBy $ UniqueCompanyGroup name
                    case mExisting of
                        Just (Entity existingId _) | existingId /= categoryId ->
                            setMessage "Company category name already exists."
                        _ -> do
